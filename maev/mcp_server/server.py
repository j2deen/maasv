...
Exposes the maev cognition layer to any MCP client (Claude Desktop, Claude Code,
...
Run modes:
- STDIO (local):  python -m maev.mcp_server
- HTTP (remote):  MAEV_TRANSPORT=http python -m maev.mcp_server

Environment variables (all prefixed MAEV_):
- MAEV_TRANSPORT: "stdio" (default) or "http"
- MAEV_PORT: Port for HTTP transport (default 8000)
- MAEV_HOST: Host for HTTP transport (default 127.0.0.1)
- MAEV_AUTH_TOKEN: API token for HTTP auth (required for HTTP)
- MAEV_DB_PATH: Path to SQLite database (default "maev.db")
- MAEV_EMBED_PROVIDER: "ollama" (default), "voyage", or "openai"
- MAEV_EMBED_MODEL: Embedding model name
- MAEV_LLM_PROVIDER: "anthropic" or "openai"
- MAEV_LLM_API_KEY: API key for LLM provider
"""
...
logger = logging.getLogger("maev.mcp")

# Session-level client tracking for origin_interface provenance
_session_client: str = "unknown"


def _init_maev(settings) -> None:
    """Initialize the maev cognition layer from MCP settings."""
    from maev import init
    from maev.config import MaevConfig
    from maev.server.providers import create_embed, create_llm
...
def create_server(settings=None) -> FastMCP:
    ...
    """
    if settings is None:
        from maev.mcp_server.config import MCPSettings
    ...
    mcp = FastMCP(
        ...
        instructions=(
            "maev is a cognition layer providing persistent memory, a knowledge graph, "
            "and a wisdom/learning system. Call maev_memory_bootstrap at session start "
            "to load context. Use maev_memory_query to search memories, "
            "maev_memory_store to save new information, and the graph/wisdom tools "
            "to manage structured knowledge and experiential learning."
        ),
    )

    # Keep a reference for HTTP auth setup
    mcp._maev_settings = settings
    ...
    def maev_memory_bootstrap(
        ...
        from maev.core import get_tiered_memory_context, get_all_active
        ...
        if query:
            from maev.core import find_similar_memories
    ...
    def maev_memory_query(
        ...
        """
        from maev.core import find_similar_memories, search_fts
    ...
    def maev_memory_store(
        ...
        """
        from maev.core import store_memory
    ...
    def maev_memory_facts(
        ...
        """
        from maev.core import get_all_active
    ...
    def maev_memory_forget(
        ...
        """
        from maev.core import delete_memory, find_similar_memories, get_all_active
    ...
    def maev_memory_supersede(
        ...
        """
        from maev.core import supersede_memory
    ...
    def maev_graph_entity_create(
        ...
        """
        from maev.core import create_entity
    ...
    @mcp.tool(annotations={"readOnlyHint": True})
    def maev_graph_entity_get(entity_id: str) -> dict:
        """
        Get an entity by ID.
        ...
        """
        from maev.core import get_entity
    ...
    def maev_graph_entity_search(
        ...
        """
        from maev.core import search_entities
    ...
    @mcp.tool(annotations={"destructiveHint": False})
    def maev_graph_entity_find_or_create(
        name: str,
        entity_type: str,
        metadata: Optional[dict] = None,
    ) -> dict:
        ...
        """
        from maev.core import find_entity_by_name, create_entity
    ...
    @mcp.tool(annotations={"readOnlyHint": True})
    def maev_graph_entity_profile(entity_id: str) -> dict:
        """
        ...
        """
        from maev.core import get_entity_profile
    ...
    @mcp.tool(annotations={"destructiveHint": False})
    def maev_graph_relationship_add(
        subject_id: str,
        predicate: str,
        ...
        from maev.core import add_relationship
    ...
    @mcp.tool(annotations={"destructiveHint": False})
    def maev_graph_relationship_expire(relationship_id: str) -> dict:
        """
        ...
        """
        from maev.core import expire_relationship
    ...
    @mcp.tool(annotations={"destructiveHint": False})
    def maev_graph_relationship_update(
        subject_id: str,
        predicate: str,
        new_value: str,
        source: Optional[str] = None,
        ...
        """
        from maev.core import update_relationship_value
    ...
    def maev_graph_query(
        ...
        """
        from maev.core import graph_query as _graph_query
    ...
    def maev_wisdom_log(
        ...
        """
        from maev.core import log_reasoning
    ...
    def maev_wisdom_outcome(
        ...
        Args:
            wisdom_id: ID from maev_wisdom_log
        ...
        """
        from maev.core import record_outcome
    ...
    def maev_wisdom_search(
        ...
        """
        from maev.core import search_wisdom as _search_wisdom, get_relevant_wisdom
        ...
        try:
            if action_type:
                ...
                from maev.core.wisdom import get_recent_wisdom
    ...
    def maev_wisdom_feedback(
        ...
        """
        from maev.core import add_feedback
    ...
    def maev_extract(
        ...
        if llm is None:
            return {
                "status": "error",
                "message": "LLM provider not configured. Set MAEV_LLM_PROVIDER and MAEV_LLM_API_KEY.",
            }

        from maev.extraction import extract_and_store_entities
...
def run() -> None:
    """Entry point for the maev-mcp script and python -m maev.mcp_server."""
    from maev.mcp_server.config import MCPSettings

    settings = MCPSettings()
    mcp = create_server(settings)

    logger.info("Starting maev MCP server...")
    logger.info("Transport: %s", settings.transport)

    if settings.transport == "http":
        if not settings.auth_token:
            print("[maev-mcp] ERROR: MAEV_AUTH_TOKEN required for HTTP transport", file=sys.stderr)
            sys.exit(1)
