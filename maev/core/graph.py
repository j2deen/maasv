...
from maev.core.db import _db, _record_entity_access, _escape_like
...
def merge_entity(keeper_id: str, duplicate_ids: list[str]) -> dict:
    ...
    from maev.lifecycle.memory_hygiene import _deduplicate_relationships
...
def search_entities(
    ...
    2. Trigram FTS5 (entities_trigram): substring matching ("Robot" in "MaevTestRobot")
       Requires query length >= 3 (trigram minimum)
    3. LIKE fallback: catches everything else (short queries, FTS errors)
    """
    from maev.core.db import _sanitize_fts_input
...
def add_relationship(
    ...
    import maev
    allowed = VALID_PREDICATES | maev.get_config().extra_predicates
    ...
    with _db() as db:
        ...
        functional = predicate in (
            FUNCTIONAL_PREDICATES | maev.get_config().extra_functional_predicates
        )
