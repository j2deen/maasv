...
def _redact_text(text: Optional[str]) -> Optional[str]:
    ...
    if text is None:
        return None
    import maev
    redact = maev.get_config().redact_output
    if redact is None:
        return text
    try:
        return redact(text)
...
def _redact_memories(memories: list[dict]) -> list[dict]:
    ...
    import maev
    if maev.get_config().redact_output is None:
        return memories
    redacted = []
    for mem in memories:
        m = dict(mem)
...
def _find_memories_by_ppr(
    ...
    from maev.core.ppr import personalized_pagerank
    try:
...
def find_similar_memories(
    ...
    import maev
    protected = maev.get_config().protected_categories
    ...
    with _db() as db:
        ...
        if maev.get_config().graph_retrieval == "ppr":
        ...
        from maev.core.reranker import rerank as ce_rerank
        ...
        from maev.core.learned_ranker import score as learned_score
        ...
        if lr_result is not None:
            ...
            primary, supplementary = _importance_score(
                candidates, protected, now, vector_distances, bm25_ids, graph_ids,
                rrf_weight=maev.get_config().rrf_rank_weight,
            )
        ...
        try:
            from maev.core.learned_ranker import log_retrieval
            log_retrieval(
                query=query,
...
def _get_category_priority() -> dict[str, int]:
    ...
    import maev
    return maev.get_config().category_priority
