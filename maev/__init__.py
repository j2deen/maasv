"""
maev — Cognition layer for AI agents.
...
Usage:
    ...
    config = MaevConfig(db_path=Path("data/my.db"))
    maev.init(config, llm=my_llm_provider, embed="ollama")

    # Now use maev.core.store, maev.core.wisdom, etc.
"""
...
def init(
    ...
    *,
    ...
    """
    Initialize maev with configuration and providers.

    Must be called before using any maev functionality.
    ...
    from maev.core.db import init_db
    init_db()

    from maev.core.wisdom import ensure_wisdom_tables
    ensure_wisdom_tables()


def _validate_embed(embed: EmbedProvider, config: MaevConfig) -> None:
    """Check that the provider returns vectors matching config expectations."""
    try:
        vec = embed.embed("maev validation")
    ...
    if abs(norm - 1.0) > 0.05:
        _log.warning(
            ...
            "maev thresholds assume normalized vectors — retrieval quality "
            "may degrade.",
            norm,
        )


def get_config() -> MaevConfig:
    """Get the current config. Raises if not initialized."""
    if not _initialized or _config is None:
        raise RuntimeError("maev not initialized. Call maev.init() first.")
    return _config
